...
- Payment gateway configuration (PayPal)
...
SETTING_SCHEMA = {
    ...
    'payment_paypal': {
        'keys': ['client_id', 'client_secret', 'webhook_id', 'api_url', 'mode', 'primary_email', 'enabled'],
        'sensitive': ['client_id', 'client_secret', 'webhook_id'],
        'required_role': 'Super Admin'
    },
    'aws_cognito': {
...
def export_settings(event, admin_user, dynamodb):
    ...
    try:
        # Scan for all settings - acceptable for admin-only export operation
        # Settings table is small (<100 items) and this runs infrequently
        response = table.scan()
...
def test_paypal_connection(dynamodb, event):
    ...
    try:
        ...
        client_secret_resp = table.get_item(Key={'setting_category': 'payment_paypal', 'setting_key': 'client_secret'})

        if 'Item' not in client_id_resp or 'Item' not in client_secret_resp:
            return create_response(200, {
                'success': False,
                'service': 'paypal',
        ...
        client_id = decrypt_value(client_id_resp['Item'].get('value', ''))
        client_secret = decrypt_value(client_secret_resp['Item'].get('value', ''))
            ...
            if 'access_token' in result:
                return create_response(200, {
                    ...
                    'details': {
                        ...
                        'scope': result.get('scope', 'N/A')[:100] + '...'
                    }
                }, event)
