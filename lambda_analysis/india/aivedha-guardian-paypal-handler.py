    ...
    try:
        ...
        order_payload = {
            ...
            "application_context": {
                ...
                "shipping_preference": "NO_SHIPPING",
                "user_action": "PAY_NOW",
                # Payment method preferences - allow guest checkout with card
                "payment_method": {
                    "payee_preferred": "UNRESTRICTED"
                },
...
def get_user_transactions(event, cors_headers):
    """Get user transaction history from the transactions table"""
    try:
        ...
        if not user_id:
            return {
                'statusCode': 400,
                'headers': cors_headers,
                'body': json.dumps({'success': False, 'error': 'userId is required'})
            }

        # Query transactions table using GSI for performance
        transactions_table = dynamodb.Table(TRANSACTIONS_TABLE)

        # Use GSI for user's transactions query (much faster than scan)
        ...
        for item in response.get('Items', []):
            ...
            txn_type = item.get('type', 'payment')
            ...
            if txn_type == 'subscription':
                ...
                pack = details.get('pack', 'Unknown')
                description = f"Credit Pack - {pack}"
            elif txn_type == 'addon':
                addon = details.get('addon', 'Unknown')
                description = f"Add-on - {addon.title()}"
            elif txn_type == 'refund':
                description = "Refund"
            else:
                description = f"Payment - {txn_type.title()}"

            # Get amount from details or set based on plan
            amount = details.get('amount') or details.get('price')
            if amount and isinstance(amount, Decimal):
                amount = float(amount)

            credits = details.get('credits')
            if credits and isinstance(credits, Decimal):
                credits = int(credits)
            ...
            transactions.append({
                ...
                'details': {
                    ...
                    if k not in ['api_key', 'token', 'secret']  # Don't expose sensitive data
                }
            })
...
def lambda_handler(event, context):
    ...
    elif '/paypal/transactions' in path and http_method == 'GET':
        # Get user transaction history
        return get_user_transactions(event, CORS_HEADERS)
