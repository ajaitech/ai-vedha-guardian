...
PAYPAL_HANDLER_URL = os.environ.get('PAYPAL_HANDLER_URL', 'https://api.aivedha.ai/api/paypal')
...
def handle_webhook(event, cors_headers):
    ...
    try:
        ...
        if event_type == 'subscription_activated':
            ...
            if user_id:
                update_user_subscription(user_id, plan_code, subscription_id, 'active', credits)
        ...
        # Handle subscription cancellation
            ...
            if user_id:
                ...
                disable_user_api_keys(user_id, 'subscription_cancelled_webhook')
        ...
        # Handle credit purchase
            ...
            if user_id and credits > 0:
                add_credits_to_user(user_id, credits, 'Credit Pack Purchase', transaction_id)
...
def handle_addon_purchase(body, cors_headers):
    ...
    print(f"Addon purchase: addon_type={addon_type}, action={action}, user_id={user_id}")

    # Handle scheduler addon
    if addon_type == 'scheduler' or action in ['create', 'update', 'delete', 'toggle', 'list', 'validate_addon', 'run_now']:
...
def handle_scheduler_addon(body, action, cors_headers):
    ...
    if action == 'create':
        ...
        start_date = body.get('start_date') or now[:10]
        start_time = body.get('start_time', '09:00')
        end_date = body.get('end_date')
        schedule_name = body.get('name', body.get('schedule_name', f"Audit for {url}"))

        schedule_id = f"sched-{uuid.uuid4().hex[:12]}"
        next_run = f"{start_date}T{start_time}:00" if start_date and start_time else now
        ...
        return json_response(200, {'success': True, 'message': 'Schedule updated', 'schedule_id': schedule_id})
        ...
        return json_response(200, {'success': True, 'message': 'Schedule deleted', 'schedule_id': schedule_id})
        ...
        return json_response(200, {'success': True, 'schedule_id': schedule_id, 'status': new_status})
        ...
        try:
            ...
            return json_response(200, {'success': True, 'message': 'Audit started', 'schedule_id': schedule_id})
        except Exception as e:
...
def handle_credits_addon(body, action, cors_headers):
    ...
    if action == 'add_credits':
        if not user_id or credits <= 0:
            return json_response(400, {'error': 'user_id and positive credits required'})

        success = add_credits_to_user(user_id, credits, f"Credits pack: {pack_id}")
        if success:
            user = get_user_by_email(user_id)
            return json_response(200, {
                'success': True,
                'new_balance': int(user.get('credits', 0)) if user else credits
            })
...
def handle_whitelabel_addon(body, action, cors_headers):
    ...
    user_id = body.get('user_id')
    brand_name = body.get('brand_name', body.get('brandName', ''))
    domain = body.get('domain', '')

    if action in ['configure', 'update']:
        if not user_id or not brand_name:
            return json_response(400, {'error': 'user_id and brand_name required'})
        ...
        users_table.update_item(
            ...
            ExpressionAttributeValues={
                ':config': {'brand_name': brand_name, 'domain': domain, 'enabled': True},
                ':now': datetime.utcnow().isoformat()
            }
        )
        return json_response(200, {
            'success': True,
            'config': {'brand_name': brand_name, 'domain': domain}
        })
