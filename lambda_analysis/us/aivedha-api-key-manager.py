...
def create_api_key(event: dict) -> dict:
    ...
    try:
        ...
        validity_days = int(body.get('validity_days', 7))

        # Validate required fields
        if not user_id:
            return json_response(400, {'error': 'User ID required'})
        if not key_name:
            return json_response(400, {'error': 'API key name required'})
        if not reason:
            return json_response(400, {'error': 'Reason for API key required'})
        ...
        if not subscription:
            return json_response(403, {
                ...
                'message': 'API key creation is available for users with active paid plans only.'
            })
        ...
        table.put_item(Item={
            ...
            'status': 'active',
            'permissions': ['audit:read', 'audit:create'],  # Limited permissions
            'usage_count': 0,
            'last_used_at': None,
        ...
        return json_response(201, {
            ...
            'usage': {
                'header': 'X-API-Key',
                'example': f'curl -H "X-API-Key: {display_key}" https://api.aivedha.ai/api/cicd/audit'
            }
        })
...
def handle_cicd_audit(event: dict) -> dict:
    ...
    try:
        ...
        if not url:
            return json_response(400, {'error': 'URL to audit is required'})
        ...
        return json_response(500, {'error': 'Audit request failed'})
